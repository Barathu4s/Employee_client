...
//     email: employeeValidationSchema.fields.email.test(
//       'unique-email',
//       'Email must be unique',
//       async function(value) {
//         return apiService.isEmailUnique(value, employee?.employeeId);
//       }
//     )
//   });
...
const EmployeeForm = ({ employee, onSubmit, onCancel, isEditing = false }) => {
  ...
  const validationSchema = Yup.object({
    ...employeeValidationSchema.fields,
    // Uniqueness is checked on submit to avoid API calls on every keystroke
    image: (() => {
  ...
  const handleSubmit = async (values, { setSubmitting, setFieldError }) => {
    try {
      // Check uniqueness once on submit
      const unique = await apiService.isEmailUnique(values.email, employee?.employeeId);
      if (!unique) { setFieldError('email', 'Email must be unique'); setSubmitting(false); return; }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFieldValue('document', e.currentTarget.files?.[0] || null)}
                  className="w-full"
                />
